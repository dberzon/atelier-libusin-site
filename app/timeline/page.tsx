  ...
  return (
    <section>
      ...
      <ul className="space-y-4">
        {items.map((i) => (
          <li key={i.year + i.title} className="p-4 border rounded-2xl">
            <div className="text-sm opacity-70">{i.year}</div>
            <div className="font-medium">{i.link ? <a href={i.link} target="_blank" rel="noreferrer">{i.title}</a> : i.title}</div>
          </li>
        ))}
      </ul>
    </section>
